...
#  ------------------------------------------------------------------------
#  2021.11.10
#  !!!!!!!!!!!!!!!!!!!!!!!!! this is a modified copy of the original nmapparser.py script from nmmapper github profile  <<<<<<<<<<<<<<<<<----------
#  modifications has been applied to filter_top_ports() method. The aim was to create JSON format for MongoDB document, wich can be query easier.  
...
class NmapCommandParser(object):
    ...
    def filter_top_ports(self, xmlroot):
        ...
        --------2021.11 
        letrehozva egy addresses array, amiben a dict objektumok felsorolsra kerulnek + 
        minden objektum kiegeszul egy IPaddr mezovel ami az IP-t tarolja. Igy elkerulve
        azt, hogy olyan mezonev jojjon letre ami az IP cim maga --> MongoDB lekerdezeben problem ha a mezonev tartalamz . karaktert
        """
        try:
            
            port_result_dict = {}
            port_result_dict["addresses"] = []
            ...
            for hosts in scanned_host:
                ...
                ipaddress = address
                address={} # A little trick to avoid errors
                address["IPaddr"]=ipaddress
                address["osmatch"]=self.parse_os(hosts)
                address["ports"] = self.parse_ports(hosts)
                address["hostname"] = self.parse_hostnames(hosts)
                address["macaddress"] = self.parse_mac_address(hosts)
                address["state"] = self.get_hostname_state(hosts)
                port_result_dict["addresses"].append(address)
